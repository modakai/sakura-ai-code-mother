...
import { addAppApi, listAppPageVoFeaturedApi, listAppPageVoMyApi } from '@/api/appApi.ts'
...
const loadMyApps = async () => {
  ...
  try {
    const res = await listAppPageVoMyApi({
    ...
    if (res.code === 0 && res.data) {
      ...
      myAppsPage.total = res.data.totalRow || 0
    }
  } catch (error) {
...
const loadFeaturedApps = async () => {
  try {
    const res = await listAppPageVoFeaturedApi({
    ...
    if (res.code === 0 && res.data) {
      ...
      featuredAppsPage.total = res.data.totalRow || 0
    }
  } catch (error) {
